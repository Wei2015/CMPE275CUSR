...
public class TicketServiceImpl implements TicketService {
	...
	/*@Transactional
	public void trainCancel (String trainName, String date) throws Exception {
		//Check time.
		Train train = trainRepository.findByBound(trainName);
		String startTime = train.getDepartureTime();
		if (timeCheck (date, startTime, 180)) {
			return;
		}
		
		//Update train status.
		TrainStatus status = trainStatusRepository.findByTrainAndDate(train, date);
		status.setCancelled(true);
		trainStatusRepository.save(status);
